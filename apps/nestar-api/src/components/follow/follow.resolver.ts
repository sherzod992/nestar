...
export class FollowResolver {
	...
	@UseGuards(AuthGuard)//login bolganlar foydalana oladi
	@Mutation((returns) => Follower)
	...
	public async unsubscribe(@Args('input') input: string, @AuthMember('_id') memberId: ObjectId): Promise<Follower> {
		...
		const followingId = shapeIntoMongoObjectId(input);// string ==> ObjectId
		return await this.followService.unsubscribe(memberId, followingId);// ikkita argument
	}
